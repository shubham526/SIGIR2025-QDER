...
An earlier version of the code and data uploaded to this repository contained an error that affected reproducibility. 
This has since been identified and corrected. 
The current code and data on this repository are correct and should reproduce the results reported in the paper. 
We encourage users to use the current code and data and to open a GitHub issue if they encounter any difficulties reproducing the results.

**Important:** Reproducing our results requires matching the following pipeline components exactly. Differences in any of these will affect results:

- Entity Linker: We use WAT with specific parameter settings that yield ~116 entities per document on Robust04. Using different linker settings or a different linker entirely will significantly affect entity coverage and downstream performance.
- Entity Ranker: We use a BERT-based entity ranker trained on entity descriptions from DBpedia. Using a different entity ranking method is not equivalent and will produce different results.
- Initial Candidate Set (BM25 run): We use a tuned BM25+RM3 run. BM25 runs generated by different toolkits (Pyserini, Pyterrier, Lucene) or with different parameter settings will produce different candidate sets and therefore different final results. We provide our exact BM25 run file in this repository — we strongly recommend using it directly rather than regenerating it.
